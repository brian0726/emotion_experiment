...
# 감정별 미디어 파일 ID (Google Drive)
# 실제 사용 시 각 감정에 맞는 파일 ID로 교체해야 합니다
GIFS = {
    # 기본 정서
    "기쁨": "PLACEHOLDER_ID",
    "분노": "PLACEHOLDER_ID",
...
# Google Drive 이미지/동영상 URL 생성
def get_media_url(emotion, media_type='gif'):
    file_id = GIFS.get(emotion)
    if not file_id:
        return None
...
def practice_intro_screen():
    ...
    instructions_text = {
        1: "다음의 예시를 통해 연습을 해 보겠습니다. 사진에 가장 적합한 형용사를 선택해 주세요.",
        2: "다음의 예시를 통해 연습을 해 보겠습니다. 동영상에 가장 적합한 형용사를 선택해 주세요.",
        3: "다음의 예시를 통해 연습을 해 보겠습니다. 이미지와 동영상을 보고 가장 적합한 형용사를 선택해 주세요."
    }
    ...
    if st.button("연습 시작", use_container_width=True):
        # 연습용 감정 선택
        practice_emotion = random.choice(list(GIFS.keys()))
...
def experiment_screen():
    ...
    if st.session_state.trial_start_time:
        elapsed = int(time.time() - st.session_state.trial_start_time)
    ...
    instruction_texts = {
        ...
        3: "다음의 이미지와 동영상을 주의 깊게 봐 주세요. 가장 적합한 형용사를 선택해 주세요."
    }
    ...
    # 미디어 표시
    media_url = get_media_url(emotion)
    if media_url:
        if exp_type == 2:  # 동영상
            st.video(media_url)
        else:  # 이미지
    ...
    # 3열로 선택지 배치
    cols_per_row = 3
    for i in range(0, len(choices), cols_per_row):
        cols = st.columns(cols_per_row)
        for j, choice in enumerate(choices[i:i+cols_per_row]):
            with cols[j]:
                if st.button(choice, key=f"choice_{i+j}", use_container_width=True):
                    handle_choice(choice, emotion, is_practice)
...
def main_intro_screen():
    ...
    과제는 총 21문항으로 약 3분 간 소요될 예정입니다.
    </div>
    """, unsafe_allow_html=True)

    if st.button("본 실험 시작", use_container_width=True):
        # 본 실험 문항 준비
        emotions = list(GIFS.keys())
        trial_order = random.sample(emotions, min(21, len(emotions)))
...
def rest_screen():
    ...
    st.markdown('<div class="instructions">휴식 시간입니다. 30초간 휴식 후 다음 과제가 시작될 예정입니다.</div>', unsafe_allow_html=True)
...
def next_part_screen():
    ...
    texts = {
        ...
        3: "두 번째 실험이 완료되었습니다.\n\n이제 마지막 실험을 시작합니다.\n이미지와 동영상을 주의 깊게 관찰하고 감정을 선택해 주세요."
    }
...
def completion_screen():
    ...
    if filename:
        st.success(f"결과가 성공적으로 저장되었습니다: {filename}")
